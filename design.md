...
**清理方法**：用户执行 `dpkg -r <removed-package>` 清理不再需要的子包。
自动清理方案：新版 postinst 通过 `upgrade` 参数检测旧版 manifest，执行差异卸载。
```
...
* 默认绑定 `127.0.0.1`，不暴露到外部网络
* 生产环境置于反向代理之后（如 nginx），由代理层处理认证和 TLS
